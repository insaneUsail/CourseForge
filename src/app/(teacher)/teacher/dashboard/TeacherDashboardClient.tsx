...
export default function TeacherDashboardClient({ classes, stats, user }: { classes: any[], stats: any, user: any }) {
  ...
  return (
    <div>
      ...
      <div className="relative w-full rounded-2xl md:rounded-3xl overflow-hidden mb-4 md:mb-6 h-auto border-2 md:border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] bg-[#FF6B35]">
        ...
        <div className="relative z-10 flex flex-col justify-end h-full p-4 md:p-5 text-[#18102B] min-h-[120px]">
          <span className="w-fit mb-2 bg-white text-[#18102B] font-black text-[10px] md:text-xs uppercase tracking-widest px-2 py-1 border-2 border-black rounded shadow-[2px_2px_0px_rgba(0,0,0,1)]">
            TEACHER PORTAL
          </span>
          <h1 className="text-xl md:text-2xl sm:text-3xl font-black mb-2 uppercase tracking-tighter text-white leading-none" style={{ WebkitTextStroke: '1px black' }}>
            Welcome back,<br className="md:hidden" /> {user?.name}
          </h1>
          <p className="text-sm text-[#18102B] font-bold bg-[#C6FF3D] w-full sm:w-fit px-3 py-1.5 border-2 border-black rounded-lg shadow-[3px_3px_0px_rgba(0,0,0,1)] leading-snug">
      ...
      {/* Finance / Stats Row */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 md:p-2 mb-4 md:mb-8">
        <Card className="bg-[#B4F481] hoverable={true} p-3 md:p-4 flex flex-col justify-between h-28 md:h-32">
          ...
          <div>
            <span className="text-xl md:text-3xl font-black text-[#18102B]">{stats.totalClasses}</span>
            <p className="text-[10px] md:text-xs font-bold text-[#18102B] uppercase tracking-wider mt-1">Total Classes</p>
          </div>
        </Card>

        <Card className="bg-[#A78BFA] hoverable={true} p-3 md:p-4 flex flex-col justify-between h-28 md:h-32">
          <div className="flex justify-between items-start">
            <div className="w-12 h-12 bg-white border-2 border-black rounded-xl flex items-center justify-center shadow-[2px_2px_0px_rgba(0,0,0,1)] -rotate-3">
          ...
          <div>
            <span className="text-xl md:text-3xl font-black text-[#18102B]">{stats.totalChapters}</span>
            <p className="text-[10px] md:text-xs font-bold text-[#18102B] uppercase tracking-wider mt-1">Total Chapters</p>
          </div>
        </Card>

        <Card className="bg-[#60A5FA] hoverable={true} p-3 md:p-4 flex flex-col justify-between h-28 md:h-32">
          <div className="flex justify-between items-start">
          ...
          <div>
            <span className="text-xl md:text-3xl font-black text-[#18102B]">{stats.totalStudents}</span>
            <p className="text-[10px] md:text-xs font-bold text-[#18102B] uppercase tracking-wider mt-1">Total Students</p>
          </div>
        </Card>
        
        <Card className="bg-[#FCD34D] hoverable={true} p-3 md:p-4 flex flex-col justify-between h-28 md:h-32">
          <div className="flex justify-between items-start">
            <div className="w-12 h-12 bg-white border-2 border-black rounded-xl flex items-center justify-center shadow-[2px_2px_0px_rgba(0,0,0,1)] -rotate-6">
          ...
          <div>
            <span className="text-xl md:text-3xl font-black text-[#18102B]">{stats.classesThisMonth}</span>
            <p className="text-[10px] md:text-xs font-bold text-[#18102B] uppercase tracking-wider mt-1">Created This Month</p>
          </div>
        </Card>
      </div>

      {/* Classes Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-6 pb-2 md:pb-4 border-b-2 md:border-b-4 border-black gap-3">
      ...
      {classes.length === 0 ? (
        <div className="w-full bg-white border-2 md:border-4 border-black rounded-[24px] md:rounded-[32px] p-4 md:p-8 flex flex-col items-center justify-center text-center shadow-[4px_4px_0px_rgba(0,0,0,1)] md:shadow-[6px_6px_0px_rgba(0,0,0,1)] relative overflow-hidden">
          <div className="w-10 md:w-16 h-10 md:h-16 bg-[#18102B] rounded-xl md:rounded-2xl flex items-center justify-center mb-4 border-2 md:border-4 border-black shadow-[3px_3px_0px_rgba(0,0,0,1)] rotate-3">
            <Presentation className="w-6 h-6 md:w-8 md:h-8 text-[#FF6B35]" />
          </div>
          <h3 className="text-lg md:text-2xl font-black text-[#18102B] mb-1 uppercase tracking-tighter">No classes yet</h3>
          <p className="text-[#18102B] font-bold text-sm md:text-base mb-4">Create your first class to get started!</p>
          <Button size="md" className="gap-2" onClick={() => setIsModalOpen(true)}>
            <Plus className="w-4 h-4" /> Create New Class
          </Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4 md:p-2">
          {/* Create New Class Card */}
          <button 
            onClick={() => setIsModalOpen(true)}
            className="group relative overflow-hidden rounded-[24px] md:rounded-[32px] border-2 md:border-4 border-dashed border-[#18102B] bg-[#F5F3FF] hover:bg-[#C6FF3D] hover:border-solid hover:shadow-[4px_4px_0px_rgba(0,0,0,1)] md:shadow-[6px_6px_0px_rgba(0,0,0,1)] transition-transform duration-300 p-4 flex flex-col items-center justify-center min-h-[200px]"
          >
